                    ...
                    if(Object.keys(fields).length > 0 && !errored){
                        Object.keys(fields).map(async k => {
                            if(fields[k].constructor == Array){
                                ...
                                if(fields[k].mime.indexOf('image/') == -1 && fields[k].mime.indexOf('video/') == -1 && !(fields[k].mime.indexOf('octet-stream') != -1 && fields[k].path.indexOf('.csv') != -1) && (fields[k].mime.indexOf('pdf') == -1 || !req.session.admin)){
                                    try {
